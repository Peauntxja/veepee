...
export function HeaderCartLink({
  ...
  if (variant === "hero") {
    return (
      <Link
        href={href}
        className="relative flex min-w-[52px] flex-col items-center justify-center rounded border border-white/80 px-2.5 py-1.5 text-[10px] font-medium leading-none opacity-95 hover:opacity-100"
      >
        {totalItems > 0 ? (
  ...
  return (
    <Link
      href={href}
      className="relative flex min-w-[56px] flex-col items-center justify-center rounded border-2 border-veepee-pink px-3 py-1.5 text-[10px] font-medium leading-none hover:bg-pink-50/40"
    >
      {totalItems > 0 ? (
        <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-veepee-pink px-0.5 text-[9px] font-bold text-white">
          {totalItems > 9 ? "9+" : totalItems}
      ...
      <span className="mt-0.5 text-[10px] font-semibold tabular-nums text-veepee-pink">
        {countdown ?? label}
      </span>
    </Link>
  );
...
function HeaderIconLink({
  ...
  const className =
    variant === "hero"
      ...
      : "flex flex-col items-center gap-1 text-[10px] font-medium leading-none text-veepee-muted hover:text-black";
